...
/* ./.libs/lt-power.c - temporary wrapper executable for .libs/power
...
const char * TARGET_PROGRAM_NAME = "power"; /* hopefully, no .exe */
...
void lt_dump_script (FILE* f)
{
  fputs ("#! /bin/sh\n", f);
  fputs ("\n", f);
  fputs ("# power - temporary wrapper script for .libs/power\n", f);
  ...
  fputs ("    echo \"power:power:$LINENO: libtool wrapper (GNU libtool) 2.4.6\" 1>&2\n", f);
  fputs ("  fi\n", f);
  fputs ("}\n", f);
  fputs ("\n", f);
  ...
  fputs ("  for lt_arg\n", f);
  fputs ("  do\n", f);
  fputs ("    $ECHO \"power:power:$LINENO: newargv[$lt_dump_args_N]: $lt_arg\"\n", f);
  ...
  fputs ("\n", f);
  fputs ("      if test -n \"$lt_option_debug\"; then\n", f);
  fputs ("        $ECHO \"power:power:$LINENO: newargv[0]: $progdir/$program\" 1>&2\n", f);
